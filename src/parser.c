...
*/
{
  ...
  */
  ...
  if (pkt.eth_hdr.eth_type == 2048) {  // hex: 0x0800 (IPv4)
    ...
    memcpy(&pkt.net_hdr.ipv4_hdr.identification, packet + net_layer_start + 4, 2);
    memcpy(&pkt.net_hdr.ipv4_hdr.flags_frag_offset, packet + net_layer_start + 6, 2);
    ...
    memcpy(&pkt.net_hdr.ipv4_hdr.hdr_checksum, packet + net_layer_start + 10, 2);
  ...
  if (pkt.net_hdr.ipv4_hdr.protocol == 6) {
    uint8_t ihl = pkt.net_hdr.ipv4_hdr.version_ihl & 0x0f;  // bit mask rightmost 4 bits for IHL
    ...
    memcpy(&pkt.trans_hdr.tcp_hdr.ack_num, packet + net_layer_start + trans_layer_start + 8, 4);
    ...
    trans_layer_start = (pkt.net_hdr.ipv4_hdr.protocol == 17) ? 20 : 40;
